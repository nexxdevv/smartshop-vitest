    ...
    <main className="mx-auto max-w-3xl px-4 py-8 sm:px-6">
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-xs">
          <span className="text-xs font-bold uppercase tracking-wider text-gray-400">Buyer Identity</span>
          <h2 className="mt-1 text-lg font-bold text-gray-900">{profile?.name}</h2>
          <p className="text-sm text-gray-500">{profile?.email}</p>
        </div>

        <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-xs">
          <span className="text-xs font-bold uppercase tracking-wider text-gray-400">Default Shipping Point</span>
          <p className="mt-1 text-sm text-gray-700">
            {profile?.address ? profile.address : 'No address saved yet. Complete a checkout to store one.'}
          </p>
        </div>
      </div>

      <div className="mt-8">
        <h2 className="text-xl font-bold tracking-tight text-gray-900 mb-4">Order History ({orders.length})</h2>
        ...
        {orders.length === 0 ? (
          ...
          <div className="space-y-4">
            {orders.map((order: any, idx: number) => (
              <div key={order._id || idx} className="rounded-xl border border-gray-200 bg-white p-4 shadow-xs">
                <div className="flex flex-wrap items-center justify-between gap-2 border-b border-gray-100 pb-3">
                  <div>
                    <p className="text-xs font-semibold text-gray-400">
                      ORDER ID: <span className="text-gray-700">#{String(order._id || idx).slice(-8).toUpperCase()}</span>
                    </p>
                ...
                <ul className="mt-3 divide-y divide-gray-50">
                  {order.items.map((item: any, itemIdx: number) => (
                    <li key={itemIdx} className="flex items-center justify-between py-2 text-sm">
